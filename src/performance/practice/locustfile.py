# 필요한 기능 import 필요
from locust import HttpUser, task, run_single_user, constant_throughput, between
import random
import logging
...
class OwnerUser(HttpUser):
    ...
    @task # @task 를 사용하여, 반복 테스크로 지정. 가중치를 parameter 로 부여
    def owners(self):
        pass
        # # 랜덤 데이터 pick
    ...
    def owner_post(self):
        ...
        #               "address": "test",
        #               "city": "test", "telephone": "1234"},
        #         name="/api/owners", catch_response=True
        # ) as response:
        #     if response.status_code != 201:
        #         logging.error(f"returned status code ${response.status_code}")
        #         response.failure("Got wrong response")
        #     # json 은 dict 로 검증
        #     elif response.json()["id"] is None : # json 리턴값을 dict 로 참조
        #         response.failure("Got wrong response")
